...
function renderActs(){
  ...
  if(!rows.length){box.innerHTML='<div class="empty"><b>'+T('act.empty')+'</b>'+T('act.emptyHint')+'</div>';return;}
  box.innerHTML=rows.map(a=>{
    ...
    const link=!pr && prAll
      ? `<span class="act-link muted" title="${T('act.otherOppHint')}">${T('act.otherOpp')}</span>`
      : pr
      ...
      : `<button class="act-new" onclick="event.stopPropagation();createProjectFromAct('${a.id}')">+ ${T('act.createOpp')}</button>`;
    return `<div class="act-row" role="button" tabindex="0" onclick="openActEdit('${a.id}')"
        ...
        title="${T('act.rowHint')}">
      <div class="act-date">${new Date(a.date).toLocaleDateString(I18N.locale())}<span class="act-type">${actType(a.type)}</span></div>
      ...
      <div class="act-note">${tv(a.note)}${actPending(a)?' <span class="act-pending" title="'+T('act.pendingHint')+'">'+T('act.pending')+'</span>':''}<small>→ ${a.next}</small></div>
